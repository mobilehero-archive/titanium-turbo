  ...
  var result = {
    ...
    parse: function(input, startRule) {
      ...
      function parse_basevalue() {
		...
		if ( _.isString(result0) && result0.startsWith('~/')) {
			result0 = ALLOY_EXPR + 'WPATH("' + result0.substr(2) + '")';
		}
        return result0;
      }
	  
	  function parse_Turbo() {
		let result;
		const theValue = input.substr(pos);
		 if (theValue.startsWith('turbo.')) {
			const test = /^[^\,\s\n]*/.exec(input.substr(pos));
			if( test ){
				pos += test[0].length;
				result = ALLOY_EXPR + test[0];
			}
		  } 
		  return result;
	  }
